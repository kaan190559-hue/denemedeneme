(function () {
 ...
 const hasIdentifier = /\b[a-f0-9]{6,24}\b/i.test(text) || /\b[A-Z0-9][A-Z0-9-]{5,}\b/.test(text);
 ...
 ${member ? ` ${escapeHtml(member.label)} ` : ""}
 ${repeatNote ? ` ${escapeHtml(repeatNote)} ` : ""}
 `);
 ...
 30 günlük profil: ${escapeHtml(profile.label)} · %${profile.successRate} 
 ${profile.approvedCount}/${profile.totalRequests} toplam talep onaylı · ${profile.failedCount} başarısız · ${profile.pendingCount} bekleyen 
 Sonuçlanan başarı %${profile.resolvedSuccessRate} · Onaylı hacim ${money(profile.approvedAmount)} 
 Ortalama onay ${money(profile.averageApprovedAmount)}${profile.lastApprovedAt ? ` · Son onay ${displayTime(profile.lastApprovedAt)}` : ""} `
 : ` 30 günlük profil: Belirsiz `;
 const lines = approvals.length ? approvals.map((item, index) => `
 ${index + 1}. onay: ${money(item.amount)} 
 ${displayTime(item.completedAt)} · ${escapeHtml(item.bank || "Banka yok")}${item.account ? ` / ${escapeHtml(item.account)}` : ""} `).join("")
 : ` Son 60 dakikada daha önce onaylanan yatırım yok. `;
 popover.innerHTML = ` ${escapeHtml(alert.label)} ${escapeHtml(alert.user || "Kullanıcı")} ${profileLine}${lines}`;
 ...
 function textContainsId(text, id) {
 if (id.length >= 6 && text.includes(id)) return true;
 if (id.length >= 8) {
 for (let len = Math.min(id.length, 10); len >= 6; len--) {
 ...
 const idMatch = identifiers.some(id => textContainsId(text, id));
 ...
 ? identifiers.some(identifier => textContainsId(text, identifier))
